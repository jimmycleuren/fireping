...
class SmokeRrdGraph extends RrdGraph
{
    ...
    public function getDetailGraph(Device $device, Probe $probe, SlaveGroup $slavegroup, $start = -3600, $end = null, $type = "default", $debug = false)
    {
        ...
        $lossColors = [
            0 => ['0', '#26ff00'],
            1 => ['1/'.$probe->getSamples(), '#00b8ff'],
            2 => ['2/'.$probe->getSamples(), '#0059ff'],
            3 => ['3/'.$probe->getSamples(), '#5e00ff'],
            4 => ['4/'.$probe->getSamples(), '#7e00ff'],
            floor($probe->getSamples() / 2) => [floor($probe->getSamples() / 2).'/'.$probe->getSamples(), '#dd00ff'],
            $probe->getSamples() - 1 => [($probe->getSamples() - 1).'/'.$probe->getSamples(), '#ff0000'],
        ];
